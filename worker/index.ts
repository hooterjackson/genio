...
async function gateway(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  try {
    ...
    return jsonError(502, "Needle service is temporarily unavailable.");
  }
  if (response.status >= 300 && response.status < 400) {
    await response.body?.cancel("upstream redirects are not allowed");
    return jsonError(502, "Needle service returned an unexpected redirect.");
  }

  const responseBody = await readLimitedResponse(response);
  if (responseBody === null) return jsonError(502, "Needle service returned an oversized response.");
