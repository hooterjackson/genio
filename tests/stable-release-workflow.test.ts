...
describe("stable release workflow trust boundary", () => {
  test("loads from the default branch and is the sole contents writer", async () => {
    ...
    for (const name of names) {
      if (
        ...
        || name === "stable-release.yml"
      ) {
        continue;
      }
      expect(
        await readFile(new URL(name, workflowsDirectory), "utf8"),
        `${name} must not be able to create stable refs`,
      ).not.toMatch(/^\s+contents:\s+write\s*$/mu);
    }
  });
  ...
  test("fails closed on missing GitHub protections before any write", async () => {
    ...
    expect(provenanceIndex).toBeLessThan(pushIndex);
