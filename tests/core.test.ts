...
test("Sites gateway uses an explicit route matrix and rejects cross-site mutations", () => {
  ...
  expect(matchGatewayRoute("GET", "/health/ready")).toMatchObject({ method: "GET" });
  expect(matchGatewayRoute("GET", "/health/ready")?.owner).toBeUndefined();
  expect(matchGatewayRoute("GET", "/health/system")).toMatchObject({ method: "GET" });
  expect(matchGatewayRoute("GET", "/health/system")?.owner).toBeUndefined();
  expect(matchGatewayRoute("POST", "/health/system")).toBeNull();
  ...
  expect(matchGatewayRoute("POST", "/api/v1/runs/run-id/dependency/resume")).toMatchObject({ method: "POST" });
  expect(matchGatewayRoute("POST", "/api/v1/runs/run-id/replay-after-repair")).toMatchObject({ method: "POST" });
  ...
  expect(matchGatewayRoute("GET", "/api/v1/owner/corpus/review")).toMatchObject({ owner: true });
  expect(matchGatewayRoute("GET", "/api/v1/owner/corpus/sources")).toMatchObject({ owner: true });
  expect(matchGatewayRoute("GET", "/api/v1/owner/corpus/assertions")).toMatchObject({ owner: true });
  expect(matchGatewayRoute("GET", "/api/v1/owner/corpus/snapshots")).toMatchObject({ owner: true });
  expect(matchGatewayRoute("POST", "/api/v1/owner/corpus/observations")).toMatchObject({ owner: true });
  expect(matchGatewayRoute("POST", "/api/v1/owner/corpus/observations/promote")).toMatchObject({ owner: true });
  expect(matchGatewayRoute("POST", "/api/v1/owner/corpus/observations/observation-id/reject")).toMatchObject({ owner: true });
  expect(matchGatewayRoute("POST", "/api/v1/owner/corpus/sources/source-id/approve")).toMatchObject({ owner: true });
  expect(matchGatewayRoute("POST", "/api/v1/owner/corpus/sources/source-id/takedown")).toMatchObject({ owner: true });
  expect(matchGatewayRoute("POST", "/api/v1/owner/corpus/assertions/assertion-id/dispute")).toMatchObject({ owner: true });
  expect(matchGatewayRoute("POST", "/api/v1/owner/corpus/assertions/assertion-id/retract")).toMatchObject({ owner: true });
  expect(matchGatewayRoute("POST", "/api/v1/owner/corpus/snapshots")).toMatchObject({ owner: true });
  expect(matchGatewayRoute("POST", "/api/v1/owner/playlists/bulk-hide")).toMatchObject({ owner: true });
