...
describe("worker pipeline protocol", () => {
  test("accepts only the current explicit protocol", () => {
    ...
    expect(WORKER_PIPELINE_PROTOCOL_NUMBER).toBe(5);
    expect(isWorkerPipelineProtocolCompatible({ protocolVersion: "playlist-pipeline-v4" })).toBe(false);
  ...
  test("models the staged v4 bridge and v5 pipeline capabilities", () => {
    ...
    expect(WORKER_PIPELINE_CAPABILITY.pipelineVersions).toEqual(["legacy_v1", "catalog_first_v2"]);
